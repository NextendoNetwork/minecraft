...
const (
	// accessKey is Minecraft: Nintendo Switch Edition's NEX access key. Extracted from the
	// game binary (@0xddd877) and VERIFIED cryptographically against a real measured CONNECT
	// signature — LitePacketSignature(md5(key), md5(key)‖srvConnSig) matches the wire byte for
	// byte. MC is NEX 4.3.1, the same as MK8 (09c1c475) / different from S2 (4eb18d39) / SSBU (9587602b).
	accessKey      = "f4983d87"
	nexVersion     = 40000
	securePID      = 2
...
var (
	...
	securePort   = envOrInt("SECURE_PORT", 60006) // MK8=60003 S2=60004 SSBU=60005 → MC=60006
...
func main() {
	...
	secureEndpoint.OnConnect = func(c *nex.Connection) {
		fmt.Printf("[MC Secure] connected pid=%d id=%d addr=%s\n", c.PID, c.ID, c.RemoteAddr)
	}
	...
	go func() {
		fmt.Printf("[MC Auth] listening WSS :%d (proxyProto=%v, secure URL -> %s)\n", authPort, proxyProto, secureURL.String())
		var err error
		if proxyProto {
		...
		if err != nil {
			fmt.Printf("[MC Auth] stopped: %v\n", err)
		}
	}()

	fmt.Printf("[MC Secure] listening WSS :%d\n", securePort)
	if err := secureServer.ListenSecure(securePort, certFile, keyFile); err != nil {
		fmt.Printf("[MC Secure] stopped: %v\n", err)
	}
}
...
func logRMC(tag string) func(*nex.Connection, *nex.RMCMessage) {
	return func(c *nex.Connection, req *nex.RMCMessage) {
		fmt.Printf("[MC %s] pid=%d proto=%#x method=%d call=%d\n", tag, c.PID, req.Protocol, req.Method, req.CallID)
	}
}
